...
task.startTasks([], {},savePage, 0,72, function(){});
...
function getHtml(index,callback){
    ...
    http.get(options, function(res) {
        ...
        res.on('end', function() {
            ...
            var name,purpose,address,developer,price,telnum,houseDetail,imageUrl,map;
            var count=dom.find('div .info').length;
            for(var i=0;i<count;i++){
                imageUrl = dom.find('.photo:eq('+i+') img').first()[0].src;
                name=dom.find('.s1 .name:eq('+i+')').text().replace(/\s+/g,'');
                houseDetail=dom.find('.s1 .name:eq('+i+') a').first()[0].href;
                purpose=dom.find('.info:eq('+i+') .dot6:eq('+0+')').text().replace(/\s+/g,'');
                address=dom.find('.info:eq('+i+') .s2 font').first()[0].title;
                developer=dom.find('.info:eq('+i+') .s2 a').first()[0].innerHTML;
                price=dom.find('.anther .price:eq('+i+')').text().replace(/\s+/g,'');
                telnum=dom.find('.sf_tuan_box:eq('+i+')').text().replace(/\s+/g,'');
                map=dom.find('.info:eq('+i+') .s2 span a').first()[0].href;
                if(dom.find('.info:eq('+i+') .s2 a').length==1){developer='';}
